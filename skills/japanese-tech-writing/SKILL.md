...
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。日本語で技術書の章、草稿、記事、解説文を書くとき、または推敲・リライトするときに使用する。納品前の機械検査（禁止語、翻訳調、無生物主語、文長のメリハリ、語彙多様性、読解負荷）は `scripts/njlint.sh` を資料モードと文章モードで使い分ける。見出し規範は `~/.claude/rules/document-tone-rules.md` に委譲する。
---
...
- 見出し・コラム見出しに、区切り線（罫線 `─` U+2500 やダッシュ類）で「種別──主題」「主題──概念」のように二要素を詰め込まない。見出しは単一の自然な句にする（要素を一つに絞るか、助詞・読点でつなぐ）。コラム見出しも「基礎」「補足」のような種別名だけで済ませず、「同値関係としての分類」「ループ不変条件と帰納法」のように内容を特定する。見出しの文体の判定は `~/.claude/rules/document-tone-rules.md`（R1〜R4）が正本で、本項と食い違う場合はそちらが優先する。
...
- **名指しのない出典**：「専門家によれば」「一般に〜と言われている」「業界では〜とされる」。出典を名指しするか、文ごと削る
- **ラベルの復唱**：箇条書きで太字ラベルの直後に同じ語を繰り返す形（「**性能**：性能が改善し〜」）。ラベルは項目名だけにし、続く文には新しい情報だけを書く。ラベルが句点で終わり、続きが別の具体を述べる形（「**スキーマは TypeScript で書く。** テーブル定義は1ファイルに置く」）は問題ない
- **接続詞代わりのコロン**：列挙や例示の前に置くコロンはよいが、文中で前後をつなぐだけのコロン（「従来の自動化から来た人向けに言うと：イベントを登録するのではなく〜」）は使わない。ダッシュを避けた結果としてコロンが増えていないか点検し、文を切るか読点でつなぐ
- **意味のない範囲表現**：「AからBまで」を、AとBが同じ尺度上にないのに幅広さの演出として使わない（「要件定義から運用まで」は工程の尺度に乗るので可。「セキュリティから UX まで」は列挙にする）
...
- 同じ対象を段落内で別の語に言い換え続けない（「顧客」「クライアント」「利用企業」を一段落で回す等）。一つの語を選び、繰り返す。言い換えは修辞であり情報ではない。
- どのプロジェクト・どの製品の文書にもそのまま置ける文は、この文書について何も言っていない。その文が読者に伝える指示・事実・数値を言い直せないなら削る（「データベースが手元にある感覚」→「`.toSQL()` が送信される SQL 文字列をそのまま返す」）。
...
見出しの文体（型・装飾・数え上げ）の判定は `~/.claude/rules/document-tone-rules.md`（R1〜R4）が正本であり、本項と食い違う場合はそちらが優先する。本項は見出しが内容を特定しているかだけを扱う。
...
規則の照合だけでは、日本語として不自然な言い回しは残る。納品前に `scripts/njlint.sh` を回す。
成果物のタイプでモードを変える。

```bash
S=~/.claude/skills/japanese-tech-writing/scripts
$S/njlint.sh --mode slide <file>                # 資料: スライド、提案書、Slack投稿、xlsx のセル
$S/njlint.sh --mode prose --genre essay <file>  # 文章: 記事、note、エッセイ
$S/njlint.sh --mode prose --genre tech <file>   # 文章: 技術記事
$S/njlint.sh --mode prose --genre business <file>  # 文章: レポート、議事録、企画書
```

資料モードは、文単位で決定的に効く検出器（禁止語、翻訳調、無生物主語）と読解負荷レーンだけを残す。
資料は断片テキストの集合なので、文書統計の検出器は意味を成さず偽陽性しか出ない。

`uv` が必要（`brew install uv`）。依存は PEP723 のインライン宣言で自己完結しており、環境を汚さない。

### findings の扱い

- すべて**要確認**として出る。Pass / Fail ではない。直すか残すかは文脈で判断する
- 判断の正本は `~/.claude/rules/document-tone-rules.md` と本ファイル。検出器の指摘が正本と食い違うときは正本が勝つ
- 「読解負荷」レーンは推敲の指さしであり、自然さの判定には含めない。指摘は「この文を見ろ」であって「直せ」ではない
- 対談やQ&A形式の原稿では、話者名で `repeated_sentence_lead` が大量に出る。検出器自身が「固有名詞由来の可能性が高い」と注記するので、そのまま無視してよい
- 一文の分割や列挙の展開で字数が増えるのは正しい結果であり、短さを目的関数にしない

### 補助スクリプト

- `scripts/outline.py <file>`: 見出しと各段落の先頭文を行番号つきで抽出する。これだけを読んで論旨が通るか確かめる
- `scripts/terms.py <file>`: 専門用語候補を初出順に並べ、初出で説明されているかのヒントを付ける

### 最後の読み返し

lint が収束しても、それは既知の表層パターンが消えたことしか意味しない。次の3点を頭の中で通す。点数はつけない。
...
- `references/readability-antipatterns.md`: 悪文27種を読解負荷順に A〜J で分類したカタログ。前から当てる
- `references/readability-principles.md`: 語順、読点、一文一義、主語述語の距離、接続詞の射程
- `references/translationese.md`: 翻訳調のパターン
- `references/doctype-failure-patterns.md`: 議事録とレポートで出やすい失敗
- `references/natural-japanese-overlay.md`: **上の3ファイルを読む前に必ず読む。** ベンダリング元の記述のうち、正本と食い違うため打ち消す箇所を名指ししている
